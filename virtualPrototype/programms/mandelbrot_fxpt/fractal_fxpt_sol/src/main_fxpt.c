...
int main() {
    ...
    // add mask for instruction cache fetches
    perf_set_mask(PERF_COUNTER_2, PERF_INSTRUCTION_FETCH_MASK);
    // add mask for instruction cache misses
    ...
    // print cache fetches and misses
    perf_print_cycles(PERF_COUNTER_2, "Instruction fetches ");
